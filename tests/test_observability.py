...
def test_retrieval_functions_are_decorated_with_traceable() -> None:
    ...
    for fn in (hybrid_search_and_join, document_overview_and_join, get_full_table, ingest_bundle):
        assert hasattr(fn, "__langsmith_traceable__") or hasattr(fn, "_is_traceable") or callable(fn)
...
def test_traceable_execution_runs_cleanly_with_tracing_active() -> None:
    ...
    with (
        patch.dict(os.environ, {"LANGCHAIN_TRACING_V2": "false"}, clear=False),
