...
def test_stage3_content_moderation_blocking(monkeypatch) -> None:
    ...
    client.chat.completions.create.side_effect = [
        ...
        _groq_response('{"violation": 1, "category": "S1", "rationale": "Unsafe content."}'),
    ]
    ...
    assert client.chat.completions.create.call_args_list[1].kwargs["messages"][0]["content"] == guardrails.SAFETY_POLICY
...
def test_api_exception_handling(monkeypatch) -> None:
    ...
    with patch("security.guardrails.Groq", side_effect=RuntimeError("network unavailable")):
